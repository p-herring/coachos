...
export default async function DashboardPage() {
  ...
  const overdueCheckinItems = (overdueCheckins ?? []) as Array<{
    id: string
    full_name: string
    next_checkin_date?: string
  }>

  const recentCheckinItems = (recentCheckins ?? []) as Array<{
    client_id: string
    submitted_at: string
    clients?: { full_name: string } | null
  }>

  const draftPlanItems = (draftPlans ?? []) as Array<{
    client_id: string
    week_start: string
    clients?: { full_name: string } | null
  }>

  const upcomingPostItems = (upcomingPosts ?? []) as Array<{
    id: string
    title?: string | null
    platforms: string[]
  ...
  const overdueCount = overdueCheckinItems.length
  const unansweredCheckinCount = recentCheckinItems.length
  const draftPlanCount = draftPlanItems.length
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {overdueCount > 0 && (
          <ActionCard
            ...
            items={overdueCheckinItems.map((c) => ({
              label: c.full_name,
        ...
        {unansweredCheckinCount > 0 && (
          <ActionCard
            ...
            items={recentCheckinItems.map((c) => ({
              label: c.clients?.full_name ?? 'Unknown',
        ...
        {draftPlanCount > 0 && (
          <ActionCard
            ...
            items={draftPlanItems.map((p) => ({
              label: p.clients?.full_name ?? 'Unknown',
        ...
        {/* Upcoming posts */}
        {upcomingPostItems.length > 0 && (
          <ActionCard
            title="Upcoming Posts"
            subtitle="Scheduled for publishing"
            icon={Clock}
            iconColour="text-green-500"
            items={upcomingPostItems.map((p) => ({
              label: p.title ?? 'Untitled post',
              sublabel: `${p.platforms.join(', ')} · ${new Date(p.scheduled_at!).toLocaleString('en-AU', { dateStyle: 'short', timeStyle: 'short', timeZone: 'Australia/Perth' })}`,
